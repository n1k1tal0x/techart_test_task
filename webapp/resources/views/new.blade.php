@extends('layouts.main')

@section('content')
    <div class="NewsList">
        <div class="TitleBlock">
            <span class="NewPath">Главная / <span class="NewName">Возвращение этнографа</span></span>
            <h1>Возвращение этнографа</h1>
        </div>
        <div class="MainNewBlock">
            <div class="NewTextBlock">
                <div class="GlobalNewsText">
                    <div class="Timestamp">
                        <span>11.06.2412</span>
                    </div>
                    <h2>Cегодня с Проксимы вернулась этнографическая экспедиция Джона Голдрама.</h2>
                    <div class="NewTextContent">
                        <p>Сегодня с Проксимы вернулась этнографическая экспедиция под управлением профессора Джона Голдрама из Майхартского университета. Экспедиция работала в джунглях Туманного континента и занималась изучением неконтактного племени двухвостых зюзюмов.</p>
                        <p>Сегодня с Проксимы вернулась этнографическая экспедиция под управлением профессора Джона Голдрама из Майхартского университета. Экспедиция работала в джунглях Туманного континента и занималась изучением неконтактного племени двухвостых зюзюмов.</p>
                        <p>Сегодня с Проксимы вернулась этнографическая экспедиция под управлением профессора Джона Голдрама из Майхартского университета. Экспедиция работала в джунглях Туманного континента и занималась изучением неконтактного племени двухвостых зюзюмов.</p>
                    </div>
                    <button><svg class="defaultArrow" width="26" height="15" viewBox="0 0 26 15" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M0.293015 8.07106C-0.0975094 7.68054 -0.0975094 7.04737 0.293014 6.65685L6.65698 0.292887C7.0475 -0.0976379 7.68067 -0.097638 8.07119 0.292886C8.46171 0.683411 8.46171 1.31658 8.07119 1.7071L2.41434 7.36395L8.07119 13.0208C8.46171 13.4113 8.46171 14.0445 8.07119 14.435C7.68067 14.8255 7.0475 14.8255 6.65698 14.435L0.293015 8.07106ZM26 8.36395L1.00012 8.36395L1.00012 6.36395L26 6.36395L26 8.36395Z" fill="#841844"/>
</svg><svg class="hoverArrow" width="26" height="15" viewBox="0 0 26 15" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M0.293015 8.07106C-0.0975094 7.68054 -0.0975094 7.04737 0.293014 6.65685L6.65698 0.292887C7.0475 -0.0976379 7.68067 -0.097638 8.07119 0.292886C8.46171 0.683411 8.46171 1.31658 8.07119 1.7071L2.41434 7.36395L8.07119 13.0208C8.46171 13.4113 8.46171 14.0445 8.07119 14.435C7.68067 14.8255 7.0475 14.8255 6.65698 14.435L0.293015 8.07106ZM26 8.36395L1.00012 8.36395L1.00012 6.36395L26 6.36395L26 8.36395Z" fill="#ffffff"/>
</svg>Назад к новостям</button>
                </div>    
                <img src="https://s3-alpha-sig.figma.com/img/2c6a/2767/3fdf97a571f6cfd84397863662dadc2c?Expires=1783900800&Key-Pair-Id=APKAQ4GOSFWCW27IBOMQ&Signature=oLGsC4jpDD1pJZpCvjlD4Rx2ukwFz32zuoKjILrCvU9XFZwZqfwpT-ZbxBsSKNqKKOtj2A~-S3Aneiuh8vOUnmc37DxvTKkGuVFXJ6hSNmjR9xIDnRWr9u3rSBnQ3fJpwuPfNwadVfryO9Axbda9UeUF0zDwLnTZFsd62QG38evSawaii~uieDmzozV1DoZ5NR4b1agxFlIZqHaiGuXaEiL2NhDBYAe7CtL9WtEhcLNwnWM3zi2slVHhKqf2Qm0Ypi6ymWvYqnIs9xCzsPCiBpalFpV7eRbXy3XUF-RD2AaxURhCM2ern7b1JoNajAD1bzLko6M4UPl2~DSoQ7AVIA_" alt="">
            </div>
        </div>
    </div>
@endsection